/*
 * Amiga TF530 SD Card Driver (tf530sd.device)
 * Copyright (C) 2018, Stephen J. Leary
 * Based on code Copyright (C) 2016, Lukas F. Hartmann <lukas@mntmn.com>
...
const char DevName[]="tf530sd.device";
const char DevIdString[]="tf530sd 1.7 (18 Aug 2017)";
...
int __UserDevInit(struct Device* dev)
{
    ...
    if (cd = (struct ConfigDev*)FindConfigDev(cd,0x13D8,0x81)) {
        registers = ((uint8*)cd->cd_BoardAddr);
